...
if [[ "${XDG_CURRENT_DESKTOP:-}" == *"KDE"* ]] || pgrep -x plasmashell > /dev/null; then
    ...
    cp "$SCRIPT_DIR/sysext-gui-wrapper-gui.sh" "$HOME/.local/bin/sysext-gui-wrapper" 2>/dev/null || true
    chmod +x "$HOME/.local/bin/sysext-gui" "$HOME/.local/bin/sysext-gui-wrapper" 2>/dev/null || true
